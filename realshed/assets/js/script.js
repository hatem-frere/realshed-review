(function ($) {
	...
	if ($(".switch_btn_one").length) {
		$(".search__toggler").on("click", function () {
			// $(this).closest(".switch_btn_one").addClass("active");
	...
	if ($('.mobile-menu').length) {
		...
		var mobileMenuContent = $('.main-header .menu-area .main-menu .navigation').html();
		// Check if default menu items (Home, Sample Page) exist to avoid duplication
		var hasDefaultMenu = $('.main-header .menu-area .main-menu .navigation li.nav-item').length > 0 &&
			$('.main-header .menu-area .main-menu .navigation li.nav-item a.nav-link').filter(function () {
				return $(this).text().trim() === 'Home' || $(this).text().trim() === 'Sample Page';
			}).length === 2;

		if (!hasDefaultMenu || $('.mobile-menu .menu-outer .navigation').length === 0) {
			$('.mobile-menu .menu-box .menu-outer').html('<ul class="navigation clearfix">' + mobileMenuContent + '</ul>');
		}
	...
	function swithcerMenu() {
		if ($('.switch_menu').length) {
			...
			$('#styleOptions').styleSwitcher({
				hasPreview: true,
				fullPath: realshedSettings.colorPath,
				cookie: {
					expires: 30,
					isManagingLoad: true
				}
